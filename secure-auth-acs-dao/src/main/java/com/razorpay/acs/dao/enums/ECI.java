...
public enum ECI {
	...
	private ECI(String value, TransactionStatus status, Network network, MessageCategory messageCategory) {
		this.value = value;
		this.status = status;
		this.network = network;
	...
	public static String getValue(TransactionStatus status, Network network, MessageCategory messageCategory) {
		String result = null;
		for(ECI eci : ECI.values()){
			
			if(eci.messageCategory != null) {
				if(eci.network.equals(network) && eci.status.equals(status) && messageCategory.equals(eci.messageCategory)){
					result = eci.value;
					break;
				}
			} else {
