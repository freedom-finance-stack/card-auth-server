...
public class RangeServiceImpl implements RangeService {
  ...
  public CardRange findByPan(String pan) throws DataNotFoundException {
    ...
    if (cardRange == null) {
      throw new DataNotFoundException(
          ...
          ErrorCode.CARD_RANGE_NOT_FOUND.getDefaultErrorMessage());
    }
    return cardRange;
  }
  ...
  public void validateRange(CardRange cardRange) throws ACSException {
    ...
    if (institution == null) {
      throw new DataNotFoundException(
          ThreeDSecureErrorCode.TRANSIENT_SYSTEM_FAILURE,
          ErrorCode.INSTITUTION_NOT_FOUND.getDefaultErrorMessage());
    }
    if (institution.getStatus() != InstitutionStatus.ACTIVE) {
      throw new ACSException(
          ErrorCode.INSTITUTION_INACTIVE.getCode(),
          ErrorCode.INSTITUTION_INACTIVE.getDefaultErrorMessage());
    }
    if (cardRange.getNetwork() == null) {
      throw new DataNotFoundException(
          ThreeDSecureErrorCode.TRANSIENT_SYSTEM_FAILURE,
          ErrorCode.RANGE_GROUP_NOT_FOUND.getDefaultErrorMessage()); // todo change error code
    }
  }
}
