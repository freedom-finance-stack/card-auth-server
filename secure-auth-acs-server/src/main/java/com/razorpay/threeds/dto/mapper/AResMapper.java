...
@Mapper(uses = {HelperMapper.class, AppConfiguration.class}, componentModel = "spring", imports = {TransactionStatus.class, MessageCategory.class, Network.class})
public interface AResMapper {


    @Mapping(target = "acsChallengeMandated", source = "transaction.challengeMandated")
    @Mapping(target = "acsDecConInd", expression = "java(transaction.getTransactionStatus().equals(TransactionStatus.CHALLENGE_REQUIRED_DECOUPLED) ? \"Y\" : \"N\")")
    @Mapping(target = "acsOperatorID", expression = "java(getOperatorId(transaction, this.helperMapper.appConfiguration))")
    @Mapping(target = "acsReferenceNumber", expression = "java(appConfiguration.getApp().getAcs().getReferenceNumber())")
    ...
    @Mapping(target = "transStatus", expression = "java(transaction.getTransactionStatus().getStatus())")
    @Mapping(target = "transStatusReason", expression = "java(getTransStatusReason(areq, transaction))")
...
// todo    @Mapping acsRenderingType, AcsSignedContent  for appbased
    ...
    default String getTransStatusReason(AREQ areq, Transaction transaction) {
        ...
        if (MessageCategory.PA.getCategory().equals(areq.getMessageCategory())
            ...
            if (Network.AMEX.getValue().equals(String.valueOf(transaction.getTransactionCardDetail().getNetworkCode()))) {
                if (TransactionStatus.SUCCESS.equals(transaction.getTransactionStatus())) {
                    transaction.setTransactionStatusReason(TransactionStatusReason.MEDIUM_CONFIDENCE.getCode());
                    transStatusReason = transaction.getTransactionStatusReason();
                } else {
                    transStatusReason = transaction.getTransactionStatusReason();
                }
    ...
    default String getOperatorId(Transaction transaction, AppConfiguration appConfiguration) {
        ...
        String operatorId = "";
        if (Network.VISA.getValue().equals(String.valueOf(transaction.getTransactionCardDetail().getNetworkCode()))) {
            operatorId = appConfiguration.getApp().getAcs().getOperatorId().getVisa();
        } else if (Network.MASTERCARD.getValue().equals(String.valueOf(transaction.getTransactionCardDetail().getNetworkCode()))) {
            operatorId = appConfiguration.getApp().getAcs().getOperatorId().getMastercard();
        } else if (Network.AMEX.getValue().equals(String.valueOf(transaction.getTransactionCardDetail().getNetworkCode()))) {
