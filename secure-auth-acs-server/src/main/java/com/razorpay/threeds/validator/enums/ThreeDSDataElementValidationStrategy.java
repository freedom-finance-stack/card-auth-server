...
public enum ThreeDSDataElementValidationStrategy implements ValidationStrategy {
	...
	THREEDS_REQUESTOR_AUTHENTICATION_INFO(ThreeDSDataElement.THREEDS_REQUESTOR_AUTHENTICATION_INFO){
		...
		public <T extends ThreeDSObject> ValidationResponse validateValue(T input) {
			...
			if (Util.isNotNull(value)) {
				if(!value.isValid()) {
					ValidationResponse v = new ValidationResponse(true, ThreeDSecureErrorCode.INVALID_FORMAT, getDataElement(), value.toString());
					return v;
				}							
			}
			return null;
	...
	MESSAGE_CATEGORY(ThreeDSDataElement.MESSAGE_CATEGORY){
		...
		public <T extends ThreeDSObject> ValidationResponse validateValue(T input) {
			...
			if(/*Utility.isNull(respnose)*/ "2.1.0".equals(objAReq.getMessageVersion())
					&& MessageCategory.PA.getCategory().equals(value)
					&& DeviceChannel.TRI.getChannel().equals(objAReq.getDeviceChannel())) {
				respnose = new ValidationResponse(true, ThreeDSecureErrorCode.INVALID_FORMAT_VALUE, getDataElement(), value);
			}
		...
		public <T extends ThreeDSObject> ValidationResponse validateValue(T input) {
			...
			if(input instanceof AREQ){
				...
				if (Util.isNotNull(value)) {
					...
					if(v != null) {
						v = new ValidationResponse(true, ThreeDSecureErrorCode.MESSAGE_RECEIVED_INVALID, getDataElement(), value);
		...
		public <T extends ThreeDSObject> ValidationResponse validateValue(T input) {
			...
			if(input instanceof AREQ){
				...
				if (Util.isNotNull(value)) {
					ValidationResponse v = validateFieldValue(value);
					if(v != null) {
						v = new ValidationResponse(true, ThreeDSecureErrorCode.MESSAGE_VERSION_NUMBER_NOT_SUPPORTED, getDataElement(), v.getValue());
					}
					return v;
				}
			}
		...
		public <T extends ThreeDSObject> ValidationResponse validateValue(T input) {
			...
			if (getDataElement().inclusion.equals(MessageInclusion.REQUIRED)
				...
				if (Util.isNull(value)) {
					ValidationResponse v = new ValidationResponse(true, ThreeDSecureErrorCode.REQUIRED_DATA_ELEMENT_MISSING, getDataElement(), value);
					return v;
				}
			}
	...
	@Override
	public <T extends ThreeDSObject, R extends ThreeDSObject> ValidationResponse validateFormat(T input, R input2,
			R input3, R input4) {
	...
	public ValidationResponse validateEmptyField(String object) {
		...
		if (Util.isNullorBlank(object)) {
			ValidationResponse v = new ValidationResponse(true, ThreeDSecureErrorCode.REQUIRED_DATA_ELEMENT_MISSING,
					getDataElement(), object);
			return v;
		} 
	...
	public ValidationResponse validateFieldLength(String object) {
		...
		if(DataLengthType.FIXED.equals(getDataElement().getLength().getLengthType())){
			...
			if (null != object && getDataElement().getLength().getLength() != object.length()) {
				ValidationResponse v = new ValidationResponse(true, ThreeDSecureErrorCode.INVALID_FORMAT_LENGTH, getDataElement(), object);
				return v;
			}
	...
	public ValidationResponse validateFieldValue(String object) {
		...
		if (!hasError) {
			ValidationResponse v = new ValidationResponse(true, ThreeDSecureErrorCode.INVALID_FORMAT_VALUE, getDataElement(), object);
			return v;
		}
		return null;
	}
