...
ABSOLUTE_PATH=$(readlink -f "$ACS_YAML_PATH")
echo "$ABSOLUTE_PATH"

# docker-compose -f ./scripts/deployment/dockerconf/card-auth-server-acs/docker-compose-dev.yaml up -d

EXTERNAL_YAML_PATH=$ABSOLUTE_PATH docker-compose -f ./scripts/deployment/dockerconf/card-auth-server-acs/docker-compose-dev.yaml up -d
