...
public class CustomECDH {
     ...
     */
    public static AlgorithmMode resolveAlgorithmMode(final JWEAlgorithm alg) throws JOSEException {
        ...
        if (alg.equals(JWEAlgorithm.ECDH_ES)) {
            ...
            throw new JOSEException(
                    AlgorithmSupportMessage.unsupportedJWEAlgorithm(
                            alg, ECDHCryptoProvider.SUPPORTED_ALGORITHMS));
        }
    }

    /**
     * Returns the bit length of the shared key (derived via concat KDF) for the specified JWE ECDH
     * algorithm.
     *
     * @param alg The JWE ECDH algorithm. Must be supported and not {@code null}.
     * @param enc The encryption method. Must be supported} and not {@code null}.
     * @return The bit length of the shared key.
     * @throws JOSEException If the JWE algorithm or encryption method is not supported.
     */
     ...
     *
     * @param publicKey The public EC key, i.e. the consumer's public EC key on encryption, or the
     *     ephemeral public EC key on decryption. Must not be {@code null}.
     * @param privateKey The private EC Key, i.e. the ephemeral private EC key on encryption, or the
     *     consumer's private EC key on decryption. Must not be {@code null}.
     * @param provider The specific JCA provider for the ECDH key agreement, {@code null} to use the
     *     default one.
     * @return The derived shared secret ('Z'), with algorithm "AES".
     * @throws JOSEException If derivation of the shared secret failed.
     */
    public static SecretKey deriveSharedSecret(
            final ECPublicKey publicKey, final ECPrivateKey privateKey, final Provider provider)
            throws JOSEException {
        ...
        try {
            ...
            throw new JOSEException(
                    "Couldn't get an ECDH key agreement instance: " + e.getMessage(), e);
        }
     ...
     *
     * @param header The JWE header. Its algorithm and encryption method must be supported. Must not
     *     be {@code null}.
     * @param Z The derived shared secret ('Z'). Must not be {@code null}.
     * @param concatKDF The concat KDF. Must be initialised and not {@code null}.
    ...
    public static SecretKey deriveSharedKey(
            final JWEHeader header, final SecretKey Z, final ConcatKDF concatKDF)
            throws JOSEException {

        final int sharedKeyLength =
                sharedKeyLength(header.getAlgorithm(), header.getEncryptionMethod());
